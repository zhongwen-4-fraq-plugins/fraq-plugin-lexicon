...
SUMMARY: Logic mode is explicit; conditions support strict boolean `or/and`, membership `in/notin`, and exact two-value `等于/不等于`; counted loops support variable counts, nesting, nearest-loop break/continue, and a 10000-iteration limit; user-input terms keep per-request timeout behavior; `[逻辑.休眠.<秒数>]` waits in answer text; `[词库.拒绝执行]` stops the current answer without compatibility aliases.
...
- Every `判断` or `否则判断` marker must be followed immediately by one `[逻辑.or]`, `[逻辑.and]`, `[逻辑.in]`, `[逻辑.notin]`, `[逻辑.等于]`, or `[逻辑.不等于]` term.
...
- Inside conditions, `or` and `and` require boolean arguments, `in/notin` test whether the first argument belongs or does not belong to later candidates, and `等于/不等于` compare exactly two non-empty rendered values.
- `等于/不等于` use exact text comparison without trimming or case normalization.
- `[逻辑.in]`, `[逻辑.notin]`, `[逻辑.等于]`, and `[逻辑.不等于]` are rejected outside a condition block.
