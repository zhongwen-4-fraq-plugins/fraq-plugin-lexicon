...
- `pnpm start:no-install` does not stop an existing Fraq instance; when startup fails with `EADDRINUSE`, identify the listening port owner, stop only the verified Fraq CLI and service processes, then rerun the command.
- Update the plugin entry in `versions.yml` before `pnpm start:no-install`; the command consumes that version to refresh metadata and cache, but does not infer or rewrite it from the active node_modules package.
