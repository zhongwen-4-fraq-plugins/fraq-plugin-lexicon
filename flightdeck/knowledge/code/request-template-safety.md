# 网络请求词条安全边界
SUMMARY: 网络请求词条只允许公网 HTTP/HTTPS，必须限制方法、JSON 参数、响应大小、重定向和超时时间，并在 DNS 解析后再次拦截私网地址；`result=json` 必须验证响应 JSON。
...
- `result` 只接受 `text` 或 `json`，默认 `text`；`json` 必须解析响应内容，解析失败返回明确错误。
