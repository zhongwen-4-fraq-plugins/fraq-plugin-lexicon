...
- 当前源码新增 `[词库.拒绝执行]`：终止本次回答，保留之前输出和动作，跳过后续文本与词条；尚未发布或安装到目标 Fraq。
- 当前源码新增 `[逻辑.休眠.<秒数>]`：在回答中按秒等待后继续执行，支持小数秒；尚未发布或安装到目标 Fraq。
- 当前发行目标为 `v0.3.4`，包含上述两项模板控制功能；目标 Fraq 仍安装 `v0.3.3`。
