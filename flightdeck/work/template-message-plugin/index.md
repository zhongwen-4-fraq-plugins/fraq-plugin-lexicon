...
- 当前源码已将条件模式的 `[逻辑.or...]` 改为只接受完全匹配的小写 `true` 和 `false`；尚未发布安装。
...
## Strict logic.or booleans

- 条件模式的 `[逻辑.or...]` 现在只接受完全匹配的小写 `true` 和 `false`。
- 不再兼容 `TRUE/FALSE`、前后空格、`1/0`、`是/否`、`真/假`；文本模式的 `逻辑.or` 不受影响。
- 用户仅指定 `逻辑.or`，因此条件模式的 `逻辑.and` 继续保留原有布尔别名。
