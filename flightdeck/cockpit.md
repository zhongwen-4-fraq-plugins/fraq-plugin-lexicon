...
Focus: 新增 `[词库.拒绝执行]` 单次回答终止控制；本地测试已通过，待检查、构建及发布安装后真实验证。

## In flight

- `template-message-plugin` — `v0.3.3` 仍安装在目标 Fraq；本地已完成 `消息.读取` 重命名并新增 `[词库.拒绝执行]`，待后续发布安装

## Next

- 发布并安装消息段读取语法变更，在真实群聊中验证 ID 3 的“获取陌生人信息@用户”。
- 发布并安装本轮变更，在真实群聊中验证 `[词库.拒绝执行]` 后的 API 和嵌套词条不执行。
