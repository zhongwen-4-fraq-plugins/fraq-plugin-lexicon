...
export type TemplateTerm =
  ...
  | { type: 'logic'; operation: 'or' | 'and' | 'in' | 'notin' | '等于' | '不等于'; values: string[] }
...
export function parseTemplateTerm(content: string): TemplateTerm {
  ...
  if (namespace === '逻辑') {
    ...
    if (
      (operation !== 'or' && operation !== 'and' && operation !== 'in' && operation !== 'notin') ||
      unescapedParts.length < 2
    ) {
      throw new LexiconError(
        '逻辑词条格式应为 [逻辑.<or|and|in|notin>.<参数1>.<参数2>...]、[逻辑.<等于|不等于>.<参数1>.<参数2>] 或 [逻辑.请求用户输入.<提示消息>.<超时时间=秒>.<超时提示=文本>]。',
      );
    }
    return { type: 'logic', operation, values: unescapedParts };
  }
