...
function helpText(): string {
  return [
    ...
    'API 参数名必须使用 Milky 协议中的英文名称，参数名错误时会提示正确名称。',
    ...
    '[逻辑.判断][逻辑.<or|and|in>.<参数...>]内容[逻辑.否则]内容[逻辑.判断.结束] 执行条件分支。',
    '条件中的 [逻辑.or] 只接受小写 true 和 false。',
    '[逻辑.计次循环.<次数>]内容[逻辑.计次循环尾] 按次数重复执行，次数可使用变量。',
    '[循环.退出] 结束最近循环，[循环.跳过] 进入下一次迭代。',
    '[逻辑.否则判断] 和 [逻辑.否则] 可省略；条件块支持无限嵌套。',
    '事件、消息段与变量词条在问题和回答中都可使用，并支持互相嵌套。问题创建的变量可在对应回答中读取。',
    '[变量.创建.A=内容] 与 [变量.读取.A] 也可嵌套在 API 参数和返回值中。',
    '[词库.<词库名>] 使用当前消息匹配指定词库，并继续解析其回答。',
    '[词库.拒绝执行] 终止当前回答，保留之前已执行的内容并跳过后续文本和词条。',
