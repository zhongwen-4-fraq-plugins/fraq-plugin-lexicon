...
中文操作方式与 Node.js `fs.open` 标志一一对应，完整对应关系见上方“文件操作方式”表。

该词条返回文件内容，不返回可继续操作的文件句柄。因为语法中没有“写入内容”参数，写入类操作方式只执行 `fs.open` 自身的创建、截断或追加打开行为；它们不会向文件写入额外文本。文件词条只能用于回答，不能用于问题匹配。

### 随机词条

随机词条使用 Fraq `@fraqjs/plugin-random` 提供的随机数生成器，应用必须先安装并激活该插件。支持 `int`、`range`、`bool` 和 `float`：
...
- `int` 返回 `[最小值, 最大值)` 区间的整数，最大值不包含在内。
- `range` 返回 `[最小值, 最大值]` 区间的整数，最大值包含在内。
- `bool` 返回布尔值；省略概率时使用 Fraq 默认值 `0.5`，也可以传入 `0..1` 的概率。
- `float` 返回 `[0, 1)` 区间的随机 32 位浮点数，不接受最小值和最大值参数。
- `int` 和 `range` 的最小值、最大值必须是 JavaScript 安全整数，范围不合法时会返回错误；`bool` 的概率必须在 `0..1` 之间。
