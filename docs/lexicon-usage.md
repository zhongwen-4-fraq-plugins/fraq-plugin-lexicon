...
| `[逻辑.or.<参数1>.<参数2>...]` | 文本模式返回首个可用参数；条件模式执行布尔或 |
| `[逻辑.and.<参数1>.<参数2>...]` | 文本模式拼接全部参数；条件模式执行布尔与 |
...
[逻辑.否则判断][逻辑.in.目标.候选1.目标]次级条件成立
[逻辑.否则]全部条件不成立
[逻辑.判断.结束]
```

- `[逻辑.判断]` 和 `[逻辑.否则判断]` 后必须紧跟一个 `or`、`and` 或 `in` 条件词条。
- 条件中的 `or` 和 `and` 始终执行布尔逻辑，不再根据参数内容自动判断模式。
- 条件中的 `in` 判断第一个参数是否精确等于后续任意候选。
- 条件中的 `逻辑.or` 只接受完全匹配的小写 `true` 和 `false`，不接受大小写变体、前后空格或其他布尔别名。
- 条件中的 `逻辑.and` 仍支持 `true/false`、`1/0`、`是/否` 和 `真/假`；其他值会返回明确错误。
