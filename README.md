...
使用 `[逻辑.请求用户输入]` 可以先发送词条中已经解析完成的前置文本，再等待同一用户在同一群聊或私聊中的下一条消息：

```text
词库 添加 精确 问 登记名字 答 请输入名字：[变量.创建.名字=[逻辑.请求用户输入]]你好，[变量.读取.名字]！
```

- 用户回复后，输入内容会替换请求词条并继续解析后续变量、API、词库和逻辑词条。
- 多个请求词条会按出现顺序依次等待，已选中的条件分支中也可以使用请求词条。
- 用户输入会作为普通文本处理，不会被当成 API 或其他模板执行。
- 等待期间，词库管理命令优先执行且不会被消费；其他下一条消息会作为输入，不再触发其他词库。
- 默认等待 5 分钟，超时后取消并返回错误；请求词条不能放在问题或逻辑条件参数中。
