...
逻辑词条支持布尔和文本两种模式，也可以嵌套变量、事件、消息段及其他词条：
...
- `or` 的参数全部是布尔值时执行或运算，否则随机返回一个参数。
- `and` 的参数全部是布尔值时执行与运算，否则按顺序拼接全部参数。
- `in` 判断第一个参数是否等于后续任意参数，返回 `true` 或 `false`。
- 布尔值支持 `true/false`、`1/0`、`是/否` 和 `真/假`。

例如：

```text
[变量.创建.A=文本1][逻辑.or.[变量.读取.A].文本2]
[逻辑.and.[逻辑.or.false.true].true]
```
